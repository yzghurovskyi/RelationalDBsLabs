...
class EditClub(npyscreen.ActionFormV2):
    ...
    def beforeEditing(self):
        ...
        if self.value:
            ...
            club_players = self.parentApp.database.get_players_by_club(club.id)
            club_tournaments = self.parentApp.database.get_tournaments_by_club(club.id)
    ...
    def on_ok(self):
        ...
        if self.club_id:
            ...
            self.parentApp.database.update_club(club)
        else:
            club.id = self.parentApp.database.add_club(club)
        self.parentApp.database.update_players_club(club.id, free_player_ids)
        self.parentApp.switchFormPrevious()

    def on_cancel(self):
        self.parentApp.switchFormPrevious()

    def fetch_free_players(self) -> list:
        free_players = self.parentApp.database.get_players_free()
        return free_players
